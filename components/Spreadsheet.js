  ...
  render () {
    return (
      <div className='spreadsheet'>
        ...
        <Table onChange={this.handleChange} onFocus={this.handleFocus} onBlur={this.handleBlur} table={this.state.table}/>

        <div className='spreadsheet__focused'>
          <h2>{this.state.focused ? `${this.state.focused[0]} - ${this.state.focused[1]}` : null}</h2>
        </div>
      </div>
    );
  }
